...
#ifdef _COMPILE_FOR_FORTIFY
#define UINT64_PRINT		"%I64u"
#else /*_COMPILE_FOR_FORTIFY*/
#define UINT64_PRINT		"%llu"
#endif /*_COMPILE_FOR_FORTIFY*/

typedef char *(*T_GetErrorMessage)(int ErrorCode, char *Buffer);
typedef int (*T_GMP_GetDllVersionEx)(char *version, uint32 Max);
...
typedef int (*T_FP3_StartEx)(uint32 hInt, uint64 *hTrx, uint8 IsBackground, uint8 *uniqueId, int lengthOfUniqueId, uint8 *uniqueIdSign, int lengthOfUniqueIdSign, uint8 *userData, int lengthOfUserData, uint8* szHashLastDate, int timeoutInMiliseconds);
...
T_GMP_GetDllVersionEx _GMP_GetDllVersionEx;
...
int GetDllVersion(void *Lib)
{
	...
	printf("Finding 'GMP_GetDllVersionEx' function...\n");

	_GMP_GetDllVersionEx = (T_GMP_GetDllVersionEx)dlsym(Lib, "GMP_GetDllVersionEx");

	if (_GMP_GetDllVersionEx == NULL)
	{
		printf("Function couldn't Find.\n");
		printf("dlError = %s\n", dlerror());
		return -2;
	...
	printf("'GMP_GetDllVersionEx' function Found.\n");
	printf("'GMP_GetDllVersionEx' = %p\n", _GMP_GetDllVersionEx);

	memset(DllVersion, 0x00, sizeof(DllVersion));
	Result = _GMP_GetDllVersionEx(DllVersion, sizeof(DllVersion));

	printf("'GMP_GetDllVersionEx' return code = %d (0x%X) : %s\n", Result, Result, CallGetErrorMessage(Lib, Result, ErrorBuf));

	if (Result == 0)
		printf("DllVersion = '%s' (new version format)\n", DllVersion);
...
int CallFP3_StartEx(void *Lib, HINT InterfaceHandle, HTRX *pTransactionHandle, uint8 IsBackground, uint8 *uniqueId, int lengthOfUniqueId, uint8 *uniqueIdSign, int lengthOfUniqueIdSign, uint8 *userData, int lengthOfUserData, uint8* szHashLastDate, int timeoutInMiliseconds)
{
	int		Result;

	printf("Finding 'FP3_StartEx' function...\n");

	_FP3_StartEx = (T_FP3_StartEx)dlsym(Lib, "FP3_StartEx");

	if (_FP3_StartEx == NULL)
	{
		printf("Function couldn't Find.\n");
		printf("dlError = %s\n", dlerror());
		return -3;
	...
	printf("'FP3_StartEx' function Found.\n");
	printf("'FP3_StartEx' = %p\n", _FP3_StartEx);

	Result = _FP3_StartEx(InterfaceHandle, pTransactionHandle, IsBackground, uniqueId, lengthOfUniqueId, uniqueIdSign, lengthOfUniqueIdSign, userData, lengthOfUserData, szHashLastDate, timeoutInMiliseconds);

	printf("'FP3_StartEx' return code = %d (0x%X) : %s\n", Result, Result, CallGetErrorMessage(Lib, Result, ErrorBuf));
...
int DoSale(void *Lib)
{
	...
	RetCode = CallFP3_StartEx(Lib, InterfaceHandle, &TransactionHandle, 0, UniqueId, sizeof(UniqueId), NULL, 0, NULL, 0, NULL, TIMEOUT_DEFAULT);
	if (RetCode != 0)
		return RetCode;
