	...
	public Functions(GmpDriver gmpDriverObj) {
		...
		btnOk.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				GmpDll.ST_FunctionParameters stFunctionParameters = new GmpDll.ST_FunctionParameters();
				
				if(rdbtnCekmeceAc.isSelected())
				{
					retcode = gmpDriver.gmpDll.FP3_FunctionOpenDrawer(MainFrame.getCurrentInt());
					
				}
				/*else if(rdbtnBankaGunSonu.isSelected())
				{
					retcode = gmpDriver.gmpDll.FP3_FunctionBankingBatch(MainFrame.getCurrentInt(), 120 * 1000);
				}*/
				else if(rdbtnBankaParametreYukle.isSelected())
				{
					retcode = gmpDriver.gmpDll.FP3_FunctionBankingParameter(MainFrame.getCurrentInt(), 120 * 1000);
				}
				else if(rdbtnParametreYukle.isSelected())
				{
					retcode = gmpDriver.gmpDll.FP3_FunctionEcrParameter(MainFrame.getCurrentInt(), 120 * 1000);
				}
				else 
				{
					...
					stFunctionParameters.password.supervisor = GmpDriver.getNulTerminatedBytes(PASS);//stFunctionParameters.supervisorPASS = GmpDriver.getNulTerminatedBytes(PASS);
					...
					retcode = gmpDriver.FP3_FunctionReports(MainFrame.getCurrentInt(), eFunc, stFunctionParameters, 120 * 1000);
